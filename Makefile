...
install-penrose-release:
	...
	@mkdir -p /usr/local/bin
	@cp -f target/release/penrose_config /usr/local/bin
	@chmod 755 /usr/local/bin/penrose_config

.PHONY: install-penrose-debug
install-penrose-debug:
	@echo ":: Installing debug build of penrose..."
	@strip target/debug/penrose_config
	@mkdir -p /usr/local/bin
	@cp -f target/debug/penrose_config /usr/local/bin
	@chmod 755 /usr/local/bin/penrose_config
...
uninstall:
	...
	@rm -f /usr/local/bin/penrose_config
